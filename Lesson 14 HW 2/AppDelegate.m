...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    //this is Pupil
    //NSLog(@" *** FIRST HALF - %@", [text substringToIndex:(text.length/2)]);
    
    //NSLog(@" \n *** SECOND HALF - %@", [text substringFromIndex:(text.length/2)]);
    
    NSArray* array = [text componentsSeparatedByString:@"."];
    
    // this is Student
    NSMutableArray* arrayWithNSString = [[NSMutableArray alloc ] init];
    NSMutableArray* arrayWithNSStringReversed = [[NSMutableArray alloc ] init];
    
    for (NSString* sentences in array) {
        NSRange range = [sentences rangeOfString:@"NSString"];
        if (range.location != NSNotFound) {
            [arrayWithNSString addObject:sentences];
        } else {
            //NSLog(@"out of word NSString - %@", sentences);
        }
    }
    
    NSLog(@"****************   MASTER  ***************");
    
    for (NSString* selectedSentences in arrayWithNSString) {
        NSMutableString* newString = [[NSMutableString alloc] init];
        
        NSArray* arrayOneSentenceReverse = [selectedSentences componentsSeparatedByString:@" "];
        
        for (NSInteger i = [arrayOneSentenceReverse count]-1; i>=0; i--) {
            [newString appendString:@" "];
            [newString appendString:[arrayOneSentenceReverse objectAtIndex:i]];
            // or this variant [newString appendFormat:@" %@",[arrayOneSentenceReverse objectAtIndex:i]];
        }
        [arrayWithNSStringReversed addObject:newString];
    }
   
    
    NSLog(@"%@", arrayWithNSStringReversed);
    ...
    for (NSString* words in arrayOfWords) {
        lastCharacterIndex = [words length] -1;
        
        if (lastCharacterIndex>0) {
            NSString* firstString =  [[words substringToIndex:lastCharacterIndex] lowercaseString];
            NSString* secondString = [[words substringFromIndex:lastCharacterIndex] uppercaseString];
            NSString* newString = [firstString stringByAppendingString:secondString];
            [newMutableString appendFormat:@"%@ ", newString];
        }
        
    }
    
    NSLog(@"%@", newMutableString);
